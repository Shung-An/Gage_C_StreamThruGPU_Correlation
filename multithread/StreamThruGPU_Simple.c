...
*/
	...
	GPUConfig.a = d_buffer1;
	GPUConfig.blocks = g_GpuConfig.i32GpuBlocks;
	GPUConfig.skip= g_GpuConfig.u32SkipFactor;
	GPUConfig.sample_size= g_CsAcqCfg.u32SampleSize;
	GPUConfig.size= u32TransferSizeSamples;
	GPUConfig.threads = g_GpuConfig.i32GpuThreads;
	GPUConfig.h_odata= h_odata;
	GPUConfig.h_dev_a = h_dev_a;
	GPUConfig.h_dev_a2 = h_dev_a2;
	GPUConfig.dev_a = dev_a;
	GPUConfig.d_accTemp = d_accTemp;
	GPUConfig.d_accTemp2 = d_accTemp2;

	transferData.g_hSystem = g_hSystem;
	transferData.nCardIndex = nCardIndex;
	transferData.u32TransferTimeout = g_StreamConfig.u32TransferTimeout;
	transferData.u32ErrorFlag = &u32ErrorFlag;
	transferData.u32ActualLength = &u32ActualLength;
	transferData.u8EndOfData = &u8EndOfData;
	...
	while( ! ( bDone || bStreamCompletedSuccess) )
	{
		...
		GPUConfig.loopCounts= u32LoopCount;

		pthread_create(&threads1, NULL, CsStmGetTransferStatus, &transferData);
		pthread_create(&threads2, NULL, GPU_Equation_PlusOne, &GPUConfig);
		...
		//cudaStatus = GPU_Equation_PlusOne(d_buffer1,
		//	g_GpuConfig.u32SkipFactor,
		//	g_CsAcqCfg.u32SampleSize,
		//	u32TransferSizeSamples, 
		//	g_GpuConfig.i32GpuBlocks,
		//	g_GpuConfig.i32GpuThreads,
		//	u32LoopCount,
		//	h_odata, 
		//	h_dev_a, 
		//	h_dev_a2,
